...
test('Verify callback execution occurs only when expected (on first component mount, on args change)', () => {
  ...
  const {rerender: rerender1} = renderHook(useTestEffect, {
    wrapper,
    initialProps: variable,
  });
  expect(count).toBe(1);
  const {rerender: rerender2} = renderHook(useTestEffect, {
    wrapper,
    initialProps: variable,
  });
  expect(count).toBe(1);
...
test('Verify cleanup function occurs only when expected (on last component unmount, on args change)', () => {
  ...
  const {rerender: rerender1, unmount: unmount1} = renderHook(useTestEffect, {
    wrapper,
    initialProps: variable,
  });
  expect(cleanup).toBe(0);
  const {rerender: rerender2, unmount: unmount2} = renderHook(useTestEffect, {
    wrapper,
    initialProps: variable,
  });
  expect(cleanup).toBe(0);
